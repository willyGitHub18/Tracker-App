...
// Contributeurs par muscle : mêmes 3 sources que calcGlobalMuscleLoad (legacy ATHX,
// programmes générés force/gym/hyrox, séances cardio), agrégés par nom d'exercice/
// modalité. Le volume affiché est la charge RÉSIDUELLE (même décroissance SRA par
// muscle que la barre %) → la liste explique le pourcentage montré.
export function calcRawContribsByMuscle() {
  const now     = Date.now();
  const contrib = {};
  Object.keys(MUSCLE_LABELS).forEach(m => { contrib[m] = []; });

  const add = (factors, name, vol, hoursAgo) => {
    if(!vol) return;
    Object.entries(factors).forEach(([mid, factor]) => {
      if(factor < 0.2 || !contrib[mid]) return;
      const hl    = RECOVERY_HALFLIFE[mid] || 48;
      const decay = Math.pow(2, -hoursAgo / hl);
      let entry = contrib[mid].find(e => e.name === name);
      if(!entry) { entry = { name, factor, raw: 0 }; contrib[mid].push(entry); }
      entry.raw += vol * factor * decay;
    });
  };

  // 1) Programmes générés actifs (force/gym/hyrox + cardio)
  const counted = new Set();
  getAllActivePrograms().forEach(prog => {
    prog.semaines?.forEach((sem, wi) => {
      sem.jours?.forEach(day => {
        day.exercices?.forEach(ex => {
          const dedupKey = `${prog.id}|${ex.id}|${wi + 1}`;
          ...
          if(raw.cardio || ex.kind === 'cardio') {
            ...
            if(!dur) return;
            const rpe = parseFloat(raw.rpe) || 4;
            add(factors, name, dur * (rpe / 10) * CARDIO_LOAD_SCALE, hoursAgo);
            return;
          }
          ...
          add(factors, name, vol, hoursAgo);
        });
      });
    });
  });

  // 2) Legacy ATHX (press / squat / deadlift), semaine par semaine
  EXERCISES.filter(e => ['press','squat','deadlift'].includes(e.id)).forEach(ex => {
    const factors = MUSCLE_MAP[ex.id];
    if(!factors) return;
    for(let w = 1; w <= 17; w++) {
      ...
      add(factors, ex.name, vol, hoursAgo);
    }
  });

  // Arrondi + retrait des contributions négligeables (décroissance quasi complète)
  Object.keys(contrib).forEach(mid => {
    contrib[mid] = contrib[mid]
      .map(e => ({ ...e, raw: Math.round(e.raw * 10) / 10 }))
      .filter(e => e.raw >= 0.1);
  });
  return contrib;
}
