  ...
  semaine.jours.forEach(day => {
    ...
    day.exercices.forEach(ex => {
      ...
      if(ex.kind === 'cardio') {
        ...
        // Analyse post-séance (feedback RPE vs cible) + aperçu séance suivante
        if(craw.rpe || craw.durationMin != null) html += _cardioSessionAnalysis(ex, craw);
        const _cdi  = semaine.jours.indexOf(day);
        const _cnxt = prog.semaines?.[week]?.jours?.[_cdi]?.exercices?.[0];
        if(_cnxt) {
          html += `<div class="next-rec-block next-rec-hold">
            <div class="rec-line">S${week + 1} : <strong>${esc(_cnxt.scheme || '')}</strong></div>
            <div class="reason">${esc(_cnxt.typeLabel || '')} · progression planifiée (~+8 %/sem)</div>
          </div>`;
        }
        html += `</div></div>`; // cardio-ex-body + ex-block
        return; // skip normal grid
...
// Analyse post-séance cardio : feedback RPE ressenti vs cible de zone (point clé du
// polarisé — ne pas courir trop dur les jours faciles) + durée réelle vs prescrite.
function _cardioSessionAnalysis(ex, craw) {
  const loggedRpe = parseFloat(craw.rpe);
  const m  = String(ex.rpeTarget || '').match(/(\d+(?:[.,]\d+)?)\D+(\d+(?:[.,]\d+)?)/);
  const lo = m ? parseFloat(m[1].replace(',', '.')) : null;
  const hi = m ? parseFloat(m[2].replace(',', '.')) : null;
  const signals = [];

  if(isFinite(loggedRpe) && lo != null) {
    if((ex.zone || 2) <= 2) { // séance facile
      if(loggedRpe > hi + 1) signals.push({ t:'warn', x:`Séance facile ressentie à RPE ${loggedRpe} (cible ${ex.rpeTarget}). En polarisé 80/20, garde les séances faciles vraiment faciles (Z2) — sinon tu accumules de la fatigue sans le bénéfice aérobie.` });
      else signals.push({ t:'good', x:`RPE ${loggedRpe} bien dans la zone facile — parfait pour la base aérobie.` });
    } else { // séance qualité
      if(loggedRpe < lo - 1) signals.push({ t:'warn', x:`RPE ${loggedRpe} sous la cible (${ex.rpeTarget}) — tu peux pousser un peu plus la prochaine séance qualité.` });
      else if(loggedRpe > hi + 0.5) signals.push({ t:'warn', x:`RPE ${loggedRpe} au-dessus de la cible — séance très exigeante, privilégie une séance facile ou du repos ensuite.` });
      else signals.push({ t:'good', x:`RPE ${loggedRpe} dans la cible qualité — bon dosage.` });
    }
  }

  const ref = ex.totalMin || ex.duration;
  if(craw.durationMin != null && ref && craw.durationMin < ref * 0.6) {
    signals.push({ t:'info', x:`Durée ${craw.durationMin} min vs ~${ref} prévue — séance écourtée (sans gravité si ponctuel).` });
  }

  if(!signals.length) return '';
  const cls = signals.some(s => s.t === 'warn') ? 'adj-behind' : 'adj-ok';
  return `<div class="adj-box ${cls}" style="padding:8px 12px">${signals.map(s => {
    const ic  = s.t === 'good' ? '✓ ' : s.t === 'warn' ? '⚠ ' : '· ';
    const col = s.t === 'good' ? 'var(--green)' : s.t === 'warn' ? 'var(--amber)' : 'var(--text2)';
    return `<div style="display:flex;gap:6px;margin-bottom:2px"><span style="color:${col};font-weight:600;flex-shrink:0">${ic}</span><span>${esc(s.x)}</span></div>`;
  }).join('')}</div>`;
}
