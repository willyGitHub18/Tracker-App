...
function _pickFocusDrills(zoneId, lvl, older, maxN, seed) {
  ...
  const order = ['dynamic', 'static', 'pnf', 'pails_rails'];
  const byMethod = {};
  pool.forEach(d => { (byMethod[d.method] = byMethod[d.method] || []).push(d); });
  const out = [];
  for(const meth of order) {
    if(out.length >= maxN) break;
    const variants = byMethod[meth];
    if(variants && variants.length) out.push(variants[seed % variants.length]);
  }
  // Compléter si peu de méthodes disponibles
  if(out.length < maxN) pool.forEach(d => { if(out.length < maxN && !out.includes(d)) out.push(d); });
  return out;
}
...
window._mobNewRoutine = function() { _renderRoutine(_curDur, true); };
