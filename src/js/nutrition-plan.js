...
function _renderNutriDetail(id) {
  ...
  const meals = MEAL_SPLIT.map(m =>
    `<div class="p-meal-row"><div class="p-meal-name">${m.nom}</div><div class="p-meal-detail">~${Math.round(c.kcalTraining * m.pct / 10) * 10} kcal (${Math.round(m.pct * 100)}%)</div></div>`
  ).join('');
  ...
  el.innerHTML = back + `
    <div class="p-section">
      ...
      <div class="p-note">Métabolisme de base ${c.bmr} kcal (Mifflin-St Jeor) × facteur d'activité = ${c.tdee} kcal TDEE, ajusté selon l'objectif « ${obj?.label || ''} ».</div>
    </div>
    <div class="p-section">
      <div class="p-sec-title">Macronutriments (jour d'entraînement)</div>
      <div class="p-card"><div class="nutri-macros">
        <div class="nutri-macro-item"><span class="nutri-macro-label">Protéines</span><span class="nutri-macro-val">${c.prot_g} g · ${c.prot_pct}%</span></div>
        <div class="nutri-macro-item"><span class="nutri-macro-label">Glucides</span><span class="nutri-macro-val">${c.gluc_g} g · ${c.gluc_pct}%</span></div>
        <div class="nutri-macro-item"><span class="nutri-macro-label">Lipides</span><span class="nutri-macro-val">${c.lip_g} g · ${c.lip_pct}%</span></div>
      </div></div>
    </div>
    <div class="p-section">
