...
function _renderProgDetailView(prog, container) {
  ...
  // Build per-day content (all weeks for each day)
  let daysContentHtml = '';
  dayNames.forEach((day, di) => {
    let weekRows = '';
    prog.semaines.forEach(sem => {
      ...
      const phaseLabel = sem.isDeload ? 'Deload' : sem.isTaper ? 'Taper' : sem.phase;
      ...
      weekRows += `
        <div class="prog-week-row" style="border-left:3px solid ${col}">
          <div class="prog-week-header">
            ...
            <span style="font-size:11px;color:var(--text3)">${esc(phaseLabel)} · ${Math.round(sem.intensite*100)}% · RPE ${sem.rpeTarget||'—'}</span>
          </div>
          <div class="prog-ex-detail-list">${exRows}</div>
        </div>`;
    });

    daysContentHtml += `<div id="gday-${di}" class="prog-tab ${di===0?'active':''}">${weekRows}</div>`;
  });
