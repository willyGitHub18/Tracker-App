...
// Détermine si le programme nécessite des 1RM (uniquement force/hyrox avec barre)
function _needsORM() {
  const forceDomains = ['force', 'hyrox'];
  if(!forceDomains.includes(_config.domaine)) return false;
  // Barre nécessaire pour les exercices avec 1RM
  return _config.materiel.includes('barre');
}

// Retourne la liste des exercices nécessitant un 1RM selon le domaine et les exercices forcés
function _ormExercises() {
  const BARBELL_LIFTS = [
    { id:'press',    label:'Strict Press',  domains:['force','hyrox'] },
    { id:'squat',    label:'Back Squat',    domains:['force','hyrox'] },
    { id:'deadlift', label:'Deadlift',      domains:['force','hyrox'] },
  ];
  return BARBELL_LIFTS.filter(ex => ex.domains.includes(_config.domaine));
}
